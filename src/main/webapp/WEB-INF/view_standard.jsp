...
<head>
   ...
   //删除确认
     function del() {
     	...
     	if (flag) {
     		var standardId = document.getElementById("standardPurId").value;
     		
     	    window.location.href='delStandard.action?standardPurId='+standardId;
     	}
...
<div class="container" style="padding-top: 15%;padding-right: 800px;padding-left: 0px;border-left-style: solid;border-left-width: 0px;margin-left: 50px;">
    ...
    <div class="col-md-12 col-md-offset-0">
        <ul class="list-group">
            ...
            <a href="view_standard">
                <li class="list-group-item  active" style="height: 50px;text-align: center">查看标准词库</li>
            </a>
    ...
    <div class="container" style="padding-top: 0px;padding-right: 7px;padding-left: 150px;border-left-style: solid;border-left-width: 0px;margin-left: 50px;">
        ...
        <h2 class="col-md-4" style="margin-top: 0px;">标准词库</h2>
        <a id="addStandard" class="btn col-md-3 col-md-offset-4 btn-primary" href="javascript:addStan();">添加标准货物</a>
        </div>
        ...
        <table   class="table table-hover table-condensed " style="background: white;color: #1b81ea">
            ...
            <tr style="font-size: 14px;" align="center">
                ...
                <th style="text-align: center">货物所属大类</th>
                <th style="text-align: center">货物所属中类</th>
                <th style="text-align: center">货物所属小类</th>
			...
			<tr v-for="(item,index) in pageInfo.list" style="font-size: 14px;" align="center">
				<td>{{item.standardPurId}} <input id="standardPurId" type="hidden" v-model="item.standardPurId"></td>
				<td>{{item.standardPurName}} <input id="standardPurName" type="hidden" v-model="item.standardPurName"></td>
				<td>{{item.firstCategory}} <input id="firstCategory" type="hidden" v-model="item.firstCategory"></td>
				<td>{{item.secondCategory}} <input id="secondCategory" type="hidden" v-model="item.secondCategory"></td>
				<td>{{item.thirdCategory}} <input id="thirdCategory" type="hidden" v-model="item.sthirdCategory"></td>
				...
				<td>
					...
					<ul class="dropdown-menu">
						...
						<li>
							<a href="javascript:addMatch();" @click="onclick(index)">添加匹配</a>	
						</li>
					</ul>
				</div>	
	...
	data:{
	 ...
	 methods:{  
         onclick:function(index){  
         	...
         	document.getElementById("firstCategory").value=vue.pageInfo.list[index].firstCategory;
         	document.getElementById("secondCategory").value=vue.pageInfo.list[index].secondCategory;
         	document.getElementById("thirdCategory").value=vue.pageInfo.list[index].thirdCategory;
         ...
         getList : function(page) {
				...
				$.ajax({
							url : 'view_standard.action?page='+rpage,
							type : 'get',
							dataType : 'json',
							async : false,
...
$(document).ready(function(){
	$.get("view_standard.action",null,function(data){
